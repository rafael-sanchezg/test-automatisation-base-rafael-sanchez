...
└── test/
    ├── resources/
    │   ├── KarateBasicTest
    │   ├── resources/  <-- UBICACIÓN ESTÁNDAR PARA TODOS LOS FEATURES
    │   │   ├── karate-test.feature
    │   │   ├── karate-config.js
...
> **IMPORTANTE**:
> 1. Todos los archivos `.feature` DEBEN crearse en la ruta `src/test/resources/[nombre_microservicio]/`.
...
| Carpeta de features | nombre del proyecto o servicio  | src/test/java/features/tre_msa_savings_account |
...
- Agrupa features por proyecto en `src/test/java/features`.
...
Feature: BTPMCDP-118 Crear cuenta exitosamente (microservicio transversal para crear cuentas)
  ...
  Scenario: T-API-BTPMCDP-118-CA01-Crear cuenta exitosamente 201 - karate
    ...
    * def result = call read('classpath:com/features/utility/generar_compliance_id.feature@generar_compliance_id')
...
- **Ubicación obligatoria**: `src/test/java/features/[nombre_microservicio]/`
- **Ejemplo**: `src/test/java/features/tre_msa_savings_account/crearCuentaAhorros.feature`
...
- **Ubicación de funciones de utilidad**: `src/main/java/utils/`
...
4. **Verificación de features**:
    - Confirmar que existe la estructura para features en `src/test/java/features/`
    - Crear las carpetas necesarias para los microservicios que se van a utilizar
...
Responsable técnico: Juan Clavijo  
